  ...
  useEffect(() => {
    ...
    return onValue(query, (snapshot) => {
      ...
      if (snapshot.exists()) {
        ...
        // convert the object to array
        let moviesArray = Object.values(result.movies);

        // set state of movies, reverse the array, so the most recent movies are on top
        setMovies(moviesArray.slice().reverse());
    ...
    <div className={"root static z-0 " + (modalVisible ? "modal-visible" : "")}>
      ...
      <main className="main-container min-h-full p-8 z-0">
        <div className={"page page-home min-h-full" + (loading ? " page-loading" : "")}>
          ...
          {!loading && movies.length > 0 && <MoviesList movies={movies} showDetails={showModal} />}
        </div>
      </main>

      {modalVisible && <MovieModal movie={movieModalData} closeModal={closeModal} />}
    </div>
  );
};
